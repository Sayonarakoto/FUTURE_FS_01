...
function Projects({ variant = 'section', onNavigate, isActive = false, revealKey }) {
  ...
  return (
    <SectionWrapper id="projects" className="bg-white bg-grid" variant={variant}>
      <div ref={revealRef} className="space-y-10">
        <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          ...
          <a
            className="projects-cta text-xs uppercase tracking-[0.24em] text-[var(--ink-soft)] transition-colors hover:text-[var(--ink-main)]"
            href="#contact"
        ...
        <div className="grid gap-6 md:grid-cols-2">
          {projects.map((project, index) => (
            <article
              key={project.title}
              className="project-card ink-card group relative rounded-[1.25rem] p-7 shadow-sm shadow-slate-200 transition-transform duration-300 hover:-translate-y-1"
            >
